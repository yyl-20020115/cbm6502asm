...
int good_label(char *p) {
	...
	if (isdigit(*p)) {   /* must be a local symbol */
		for (i = error = 0; isdigit(*p) && *p; p++)
			i = i * 10 + *p - '0';
		if (*p++ != '$') display_error(error = 'F');
		if (*p) display_error(error = 'F');
		if (i > 255 || i < 1) display_error(error = 'F');
		return (!error);
	}
...
void pass1(char *macprmbgn, char **maclinptr, char *macargbgn) {
	...
	while (incl_sp <= MAXINCL) { /* consider nested includes */
		...
		while (get_line(macprmbgn, maclinptr, macargbgn) !=
		       EOF) { /* read to end of file or macro */
			old_loc =
			        loc_cntr; /* save location couter for listing */
			...
			tmpmac = macro_head;
			while (tmpmac && !done &&
			       condit) { /* check if this is a macro expansion
				          */
				if (!strcmp(s2, tmpmac->name)) { /* is this a
					                            macro? */
					char *p;      /* temporary use to copy
					                 argument list */
					char *prmbgn; /* pointer to beginning of
					                 parameter list */
					char *defbgn; /* pointer to beginning of
					                 macro definition */
					char *argbgn; /* pointer to beginning of
					                 argument list */
					/* use heap for argument list - don't
					 * fill up stack */
					if (*s1)
						if (good_label(s1))
							get_label(s1, 1);
					p = argbgn =
					        (char *)malloc(strlen(s3) + 50);
					if (!p)
						error_msg(
						        "Memory allocation "
						        "failure during macro "
						        "expansion.",
						        "", 4);
					prmbgn = tmpmac->name;
					while (*prmbgn++)
						; /* point to start of parameter
						     list */
					defbgn = prmbgn + 2;
					while (*(defbgn - 1) || *(defbgn - 2))
						defbgn++; /* starts just after
						             the double null */
					parses3(1);       /* argument list */
					while (*s3 ||
					       *(s3 + 1)) /* copy arg list until
					                     double null */
						*p++ = *s3++;
					*p++ = 0;
					*p = 0;
				...
				tmpmac = tmpmac->nextmac; /* not this macro -
				                             try next if any */
			}
			if (macro) continue;
			if (!done) {
				savprint = print; /* save print flag - directive
				                     changes it */
				if (!direct1(macprmbgn, maclinptr,
				             macargbgn)) /* asm directive */
					if (!direct2()) {
						print = savprint; /* restore
						                     print flag
						                     if not
						                     directive
						                   */
