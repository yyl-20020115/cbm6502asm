...
/* mactype: 1=.MACRO 2=.REPT 3=.IRP 4=.IRPC */
	...
	newmac = (struct macdef *)malloc(
	        l = CHUNK); /* initial size of structure */
	                    /*
	                      save macro name
	                    */
	for (m = i = 0; s1[i]; i++) newmac->name[m++] = s1[i]; /* copy name */
	...
	*/
	i = 1;
	while (i) {
		get_line(macprmbgn, maclinptr,
		         macargbgn); /* get a source or macro exp line */
		print = 1;

		if (strcmp(s2, ".MACRO") ==
		            0 || /* check for nested macro defs */
		    strcmp(s2, ".IRP") == 0 ||
		    strcmp(s2, ".IRPC") == 0 || strcmp(s2, ".REPT") == 0)
			macrocnt++;
		if (strcmp(s2, ".ENDM") == 0 || strcmp(s2, ".ENDR") == 0)
			macrocnt--;

		if (!strcmp(s2, ".END"))
			error_msg(
			        ".END encountered during definition of macro: ",
			        newmac->name, 4);

		spechar = 'D';
		/* check for end of macro definition */
		if (((!strcmp(s2, ".ENDM") && mactype == 1) ||
		     (!strcmp(s2, ".ENDR") && mactype > 1)) &&
		    !macrocnt) {
			newmac->name[m++] =
			        0; /* add a null indicating macro end */
			newmac->size = m + 8;
			if (error) {
				free((char *)newmac); /* don't save macro if
				                         error */
				return;
			}
			newmac->nextmac = macro_head; /* link macro definition
			                                 into list */
...
void macro_tab() {
	...
	if (macro_head)
		fprintf(file_lst,
		        "\nMemory used to store %d macro definitions: %ld. "
		        "bytes.\n",
		        i, s);
	else
		fprintf(file_lst, "\n\nNo macros defined.\n");
