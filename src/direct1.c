...
int direct1(char *macprmbgn, char **maclinptr, char *macargbgn) {
	...
	if (!strcmp(s2, ".ELSE")) {
		...
		if (condlevel) {
			...
			for (condit = TRUE, i = 1; condit && i <= condlevel;
			     i++)
	...
	if (!strcmp(s2, ".REPT")) {    /* repeat directive */
		...
		if (i) {
			...
			outline(addr = 0, data = 0, (char *)0, (char)0,
			        (char)1);
			...
			if ((tmpmac = macro_head) == reptmac)
				macro_head = reptmac->nextmac; /* unlink rept
				                                  macro */
			else {
				while (tmpmac->nextmac != reptmac)
					tmpmac = tmpmac->nextmac;
				tmpmac->nextmac =
				        reptmac->nextmac; /* unlink rept macro
				                           */
			}
			free((char *)reptmac); /* destroy temporary macro
			                          definition */
		} else
			display_error(error = 'U');
		addr = data = 0;
	...
	if (!strcmp(s2, ".IRPC")) { /* repeat directive */
		...
		while (*s3 != ',' && *s3) s3++;
		*s3 = 0;
		if (!(chars = (char *)malloc(strlen(++s3) + 2))) memallocerr();
		strcpy(chars, s3);
		s3 = p3;
	...
	if (!strcmp(s2, ".IRP")) { /* repeat directive */
		...
		if (!isspace(line[0])) /* save label if any */
			if (good_label(s1)) get_label(s1, 1);
		sprintf(p1, "___T.M.P.$$$%08.8ld", tmpcounter++);
		if (!(chars = (char *)calloc(strlen(p3), 1))) memallocerr();
		parses3(1);
		while (*s3) s3++;
		s3++;
		i = 0;
		while (*s3 || s3[1]) chars[i++] = *s3++;
		s3 = p3;
	...
	if (!strcmp(s2, ".MACRO") ||
	    !strcmp(s2, ".MAC")) { /* macro definition? */
		if (isspace(line[0])) display_error(error = 'F');
