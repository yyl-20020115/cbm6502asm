...
#define SYMBLKSZ \
	4096 /* how much memory for symbol table entries to alloc at once */
#define LEFT 0
#define RIGHT 1
#define BALANCED 2
#define LX extern
...
char *allocsym(int symlen) {
	...
	if (symbol_ptr >= SYMBLKSZ) {
		...
		if (!symbol_buf)
			error_msg("Available memory exceeded storing symbol",
			          "", 4);
		symptr = 0;
		symbol_ptr = sizeof(struct smbl) - 31 + symlen;
		symblk++;
...
void move() {
	if (direction[level] == direction[level + 1]) {
		...
		} else if (direction[level + 2] != BALANCED) {
			path[level]->weight = !direction[level];
			path[level + 1]->weight = BALANCED;
		} else
			path[level]->weight = path[level + 1]->weight =
			        BALANCED;
	}
}
...
void addref(struct xrfr **xref, int def) {
	...
	if (--xrefcnt <= 0) {
		xrefcnt = XREFALLOC;
		if (!(xrefheap = (struct xrfr *)calloc(XREFALLOC,
		                                       sizeof(struct xrfr))))
			error_msg("Available memory exceeded adding reference",
			          "", 4);
	}
...
void insert_symbol(char *sym, long value, int def, int known, int local) {
	...
	ptr->defined = def;
	ptr->known = known;
	ptr->label = ptr->equate = ptr->section = ptr->relocate =
	        ptr->multiple = ptr->external = 0;
	if (!local) addref(&ptr->xref, def);
...
void output_symbol(struct smbl *pntr) {
	...
	if (pntr->name[0] == '-')
		...
		fprintf(file_lst, "\n%c %04lX %-31s ", c, pntr->value & 0xffffL,
		        pntr->name);
		if (prnxref) prnref(pntr->xref);
	}
	if (pntr->link[1]) output_symbol(pntr->link[1]);
}

void list_sym() {
	refknt = locknt = symknt = 0;
	fprintf(file_lst, "\nErrors detected in %d line", errcount);
	if (errcount != 1) fprintf(file_lst, "s");
	fprintf(file_lst, ".\n\f\n");
	fprintf(file_lst, " value name                            ");
	if (prnxref) fprintf(file_lst, "cross reference");
	fprintf(file_lst, "\n");
	if (root_symtab) output_symbol(root_symtab);
	fprintf(file_lst,
	        "\n\nMemory used to store %d local symbols: %d. bytes.", locknt,
	        26 * locknt);
	fprintf(file_lst,
