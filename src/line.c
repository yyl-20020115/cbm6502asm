...
void parselin() {
	...
	if (j < 0) return;
	if (line[i] != ';' &&
	    line[i]) { /* don't get fields for comments & blank lines */
		if (!isspace(line[i]))
		...
		while (line[i]) *s3++ = line[i++]; /* the rest of the line */
		*s3++ = 0;
		*s3++ = 0;
		s3 = p3;
	}
...
void parses3(int enablang) {
	...
	while (*s3 && (*s3 != ';' || squote || dquote)) {
		...
		if (*s3 == '~') { /* special treatment for the tilde char */
			pnt = s3 +
			      1; /* save pointer to char after char after ~ */
		...
		if (*s3 == '\'' &&
		    !dquote)              /* if sngl quote not in dbl quotes */
			squote = !squote; /* toggle single quote status */
		if (*s3 == '\"' &&
		    !squote)              /* if dbl quote not in sngl quotes */
			dquote = !dquote; /* toggle double quote status */
		if (*s3 == '<' && !dquote &&
		    !squote) /* count angle brackets < */
			angleb++;
		if (*s3 == '>' && !dquote &&
		    !squote) /* count angle brackets > */
			angleb--;
		if (!(enablang * angleb | dquote |
		      squote)) {          /* if delimiters are balanced */
...
void macsubst(char *parm, char *arg, char *subbed) { /* substitute macro args for parms */
	...
	for (i = 0; i <= ll - lp; i++) {
		...
		if (j) continue;            /* don't replace if part of sub */
		...
		if (dif > 0)
			for (j = (int)strlen(line) + dif + 1; j > i;
				...
				subbed[j] =
				        subbed[j - dif]; /* and sub'd flags */
			}
		if (dif < 0)
			for (j = i; j < strlen(line) + dif + 1;
			     j++) { /* parm is longer than arg */
				line[j] = line[j - dif]; /* shift line */
				subbed[j] =
				        subbed[j - dif]; /* and sub's flags */
			}

		for (j = angleb; j < la - angleb;
...
int get_line(char *macprmbgn, char **maclinptr, char *macargbgn) {
	...
	error_field[0] = error_field[1] = error_field[2] = error_field[3] =
	        error_field[4] = ' ';
	if (macrolevel) {
		c = '\n';
		...
		if (*maclinp) {       /* if more lines left of this macro */
			...
			*maclinptr =
			        maclinp; /* for next call to this function */
			                 /*
			                         make substitutions
			                 */
			...
			while (*macprmptr) {   /* check if any parameters remain
				                */
				macsubst(macprmptr, macargptr,
				         subbed); /* make substitutions */
				while (*macprmptr++)
					; /* next parameter */
				if (*macargptr)
					while (*macargptr++)
		...
		for (i = 0; (c = fgetc(file_src)) != EOF; i++)
			if (c != '\n')
				if (i < LINELENG - 1)
					...
					error_msg("Error reading source file",
					          "", error = 1);
					for (c = fgetc(file_src);
					     c != EOF && c != '\n';
					     c = fgetc(file_src))
						;
				}
